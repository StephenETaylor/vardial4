...
I've decided to modify this program even though it contains a bunch of stuff
I'm not using yet, in the hope it will eventually prove useful.
Specifically, I don't have an nvidia graphics card in my laptop, so all
training uses the CPU; probably there isn't a big win doing batches of 
sequences in parallel.  But Amazon does lease online CPU instances with GPU...
...
-- SET: I use a debugger, obtained from
-- https://github.com/slembcke/debugger.lua
--  the following refers to the file debugger.lua, but I have
...
function narrow_list_matching(listOfTensors, dim1) 
    ...
    answer = {}
    for i,v in pairs(listOfTensors) do
        answer[i] = v:narrow(1,1,dim1)
    end
    return answer
end
...
function feval(x)
    ...
    if x == nil then 
        loader:reset_batch_pointer(1) -- SET if at end of split, just restart
        x, y = loader:next_batch(1)
    end
    -- x:size() is now batch_size x sequence_length
    x,y = prepro(x,y)  -- transpose x and y : put seq_len into first dimension
    ------------------- forward pass -------------------
    -- set up initial state

    -- narrow state to match size of batch
    local rnn_state = {[0] = narrow_list_matching(clone_list(init_state),
						  x:size(2))}
    -- previous line was formerly: local rnn_state = {[0] = init_state_global}
    local predictions = {}           -- softmax outputs
    local loss = 0
    local loc_seq_length = x:size(1)
    ...
    loss = loss / loc_seq_length
    ------------------ backward pass -------------------
    -- initialize gradient at time t to be zeros (there's no influence from future)
    -- adjust width of drnn_state tensors to match actual (dynamic) batch size
    local drnn_state = {[loc_seq_length] = 
                narrow_list_matching(clone_list(init_state, true), x:size(2))} -- true also zeros the clones 
    for t=loc_seq_length,1,-1 do
    ...
    -- SET in following (comment) line, changed opt.seq_length to loc_seq_length
    --  not sure, does rmsprop know opt.seq_length?
    -- grad_params:div(loc_seq_length) -- this line should be here but since we use rmsprop it would have no effect. Removing for efficiency
